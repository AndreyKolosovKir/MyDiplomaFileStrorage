...
import ru.netology.exception.DeleteFileException;
...
public class FileService {
    ...
    public boolean uploadFile(String authToken, String filename, MultipartFile file) {
        ...
        //по юзеру получнному из токена проверяем есть ли такой юзер
        if (user == null) {
            log.error("Upload file: Unauthorized");
            throw new UnauthorizedException("Upload file: Unauthorized");
        }
        //пытаемся сохранить файл используя те данные которые указаны как агрументы метода и логируем успешное действие
        try {
            ...
            return true;
            //если файл не удалось сохранить пробрасмываем исключение и логируем
        } catch (IOException e) {
